...
public class Round {
    ...
    public double startRound(QuestionBank QB, Lifeline lifeline) throws EndGameException{ 
        ...
        for(int i = 0; i < questAndCash.size(); i++){
                ...
                if(cashWon == 0){
                    throw new EndGameException("\n** Uh Oh! Galat Jawab, iska sahi uttar hain: '" + quest.getCorrectAns() + "' **\n" +
                    "* Sorry, your game ends here! *");
                }else if(cashWon == 1000000.0){
                    throw new EndGameException("\n** Woohooo!  Aap aant tak pauch gaye hain! **\n"
                            + "You are now a crorepati! Congratulations on winning " + String.format("Rs.%,.2f", cashWon) );
                }else{
                    System.out.println("\n** Sahi Jawab! Congratulations! Aap jeete " + String.format("Rs.%,.2f", cashWon) + " uptill now! **");
                }
            }        
        return cashWon;    
    }
